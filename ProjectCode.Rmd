...
* **Time**:  REC is only for the NIOSH survey data and so doesn't have a time period.  `cohist` has whole years and a variable *period* which is an integer coded from 2-8.  `jem` has both *year* and *period*.  Coding is identical for year but the *period* variable is a factor with labels rather than an integer.  To learn how `cohist` and `jem` definitions of period align, we need to do a merge and compare them.  See below.
...
# observe the periods line up in jem and cohist, though there are no data for
# the earliest period in jem
...
JEMnew <- left_join(jem, RECnew, by = c("facilityno", "job" = "job"))
...
# Different Analysis Steps when Period is the Time Variable
