...
This is the companion R Markdown file to the JEM procedure overview file: *jemsyntax2019.docx*.  This file describes the general steps for generating a Job Exposure Matrix (JEM) for REC from the DEMS data files.  It also addresses dataset features you need to be aware of during your project.  Other important aspects of the project are not described here.
...
This syntax that we provide for you is supposed to be generic – that is, it will work, but the models provided are not necessarily the best ones to use.  For instance, in the following sample code, we have fit simple models for all mines combined, and also showed you the methods you would need to predict from these models.  You need to explore and determine the best models, and interpret the results.
...
* **Time**:  REC is only for the NIOSH survey data and so doesn't have a time period.  `cohist` has whole years and a variable *period* which is an integer coded from 2-8.  `jem` has both *year* and *period*.  Coding is identical for year but the *period* variable is a factor with labels rather than an integer.

* **Job**:  `jem` has a single variable *job* which is coded the same as the variable *job* in `REC`.  `REC` has two additional job variables:  *mdj*, the concatenation of mine (as a number), department, and job, and *job98* which is not identical to *job* and TODO:  ADD explanation

Here is some code to create a factor variable for faciity in `cohist` that aligns with the simiar factor variable in the other two datasets.
...
* How you approach the modeling will affect whether you can predict for all job and facility combinations.  Think about what predictions you think are supported scientifically.  
* We have not addressed here how to handle facility J. If you are going to try to predict at this facility, how will you approach it and why?
...
REC$jobf <- relevel(REC$jobf, ref = "410")
...
f <- unique(REC$facilityno)
j <- unique(REC$jobf)
# currently doesn't work with this option: why?
#newdat <- expand.grid(faciltyno = f, jobf = j)
...
# 
RECnew <- newdat %>% 
    select(facilityno, facilityid, job, jobf, rec_AM)
...
# combinations, n = ADD
f <- unique(cohist$facilityfac)
t <- unique(cohist$year)
# currently doesn't work with this option: why?
#newdat <- expand.grid(faciltyno = f, year = t)
...
# like in the other datasets
COnew <- newdat %>%
    select(facilityno, facilityid, facilityfac, year, period, co_AM)
...
TODO:  Make sure to verify you did the merge correctly.
...
Use a version of the formula in DEMS IV to estimate the REC for each observation in the JEM dataset  
    - Formula:  rec_mjy = (CO_am / baselineco) * REC_am  
...
JEM <- JEMnew2 %>%
    mutate(rec_mjy = (co_AM / baselineco) / rec_AM)
