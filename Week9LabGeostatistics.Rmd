...
```{r read.data.from.a.directory, eval=TRUE, echo=FALSE}
...
# TODO:  this is a crude approach; look into ColorBrewer
...
Note that `geoR` appears to do *all* its distance calculations based on the input coordinates using the Pythagorean theorem.  This won't give you correct distances on a sphere.  Thus we can't use latitude and longitude directly.  The dataset provided by Laina Mercer had two variables, `lat_m` and `long_m` that had been projected to a flat surface.  At this writing this was not documented.  So the dataset we are using in this lab has [Lambert coordinates](https://en.wikipedia.org/wiki/Lambert_azimuthal_equal-area_projection) added.  These give approximately the same meters as the coordinates Mercer used.
...
Below are the estimates from the UK models, using both WLS and REML.  Note that only the REML model gives us estimates for the fixed effect parameters.  Also the REML model gives us covariance parameter estimates that are similar those reported by Mercer et al, although the range estimate is smaller.   This may be due to use of a different coordinate system. 
...
TODO:  Try to produce the variograms like Mercer.  Need to use the breaks function to allow for very fine scale distances to capture the gradient structure.
...
1.  Fit a LUR model (using the common model covariates) to the season-specific snapshot data.  (One model per season.)  Take the residuals from those models and evaluate them:
    ...
    c.  Is there evidence of spatial structure in the data?  Do you get different insights from each variogram?
    
2.  Using 10-fold cross-validation with the cluster variable to define the CV groups, estimate predictions from a 2-step model using the common model covariates.  For this model you need to separately create cross-validated predictions from the LUR model and from the OK model of the LUR residuals (of the full season-specfic dataset), and sum these to compare with the observed ln_nox data.
