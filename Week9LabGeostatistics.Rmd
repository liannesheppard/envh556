...
# funModeling:  for EDA
# ColorBrewer: color palatte for maps
pacman::p_load(tidyverse, knitr, dplyr, geoR, maps, scatterplot3d,ggmap, funModeling, Hmisc, ColorBrewer)
...
One can superimpose various modeled variograms onto empirical variograms.  This is important because we need to have some idea of appropriate variogram parameters because we need to supply the estmiation fuction (`likfit`) with initial values of these parameters, specifically the partial sill ($\sigma^2$) and range ($\phi$).  One can also allow for a non-zero nugget ($\tau^2$) parameter by using the `nugget = ` option with an estimated value after the "=".  The program defaults to estimating the nugget unless you use the `fit.nugget = TRUE` option.
...
Since we need to supply a likelihood fit with initial values of parameters, an alternative is to use the `variofit` function with no initial values and let `geoR` try to estimate these as shown next.
...
Example to show that we can get a likelihood fit using either known (guessed) initial values for the partial sill and range, or taken from the output of the `variog` function.
...
More examples of estimating parameters, now with plots to show results.
...
legend("bottomright", c("empirical: ordinary","empirical: universal", "modeled: exponential"), lty = c(1,1,1), col=1:3, text.width = 200, cex = 0.6)
...
# Universal Kriging (UK) with covariates altitude and area
# Choose the initial values using the byeye variogram
# Look at whether the results vary by initial values; if so address this.  You should evaluate the sensitivity of your results to the initial values.
# The problem in ARC GIS is they automatically assign some as defaults.  They aren't always right and can be important.
...
ca20.r2.ml <- 1 - ca20.mse.ml / 
        mean( (ca20.xv.ml$data - mean(ca20.xv.ml$data))^2 )
ca20.mse.ml
ca20.r2.ml
...
# TODO:  What kind of plot are we leveraging here? image is for kriging predictions, the other two seem to be standard plots
...
# TODO: find out what zoom to recommend
...
g <- ggmap(map, darken = c(0.5, "white")) + theme_void() + 
    ggtitle("Sample Map of Los Angeles for \n the area covered by the snapshot data")
...
g + geom_point(aes(x = longitude, y = latitude, col = ln_nox), data = fall, alpha = 0.5) + scale_color_brewer(direction = -1, palette = "YlOrRd")
# TODO:  change title or add legend
# TODO:  put ln_nox into ~7 categories in a new variable to display
# TODO:  Figure out RColorBrewer and whether to apply
# TODO:  Fix color scale on this
