...
For later use, we need to convert the grid to geodata.  Also, the full grid isn't working, so we use a subset.  TODO:  Future work: Figure out a solution. Perhaps we can make this work by dividing the prediction step into two pieces?
...
This refinement is the same gridded predictions, but now drops all points over the water.  However, currently this implementation does not work so the result is not shown.

```{r plot gridded predictions not on the water, eval = FALSE}
# ------------ plot gridded predictions not on the water ---------------
# TODO: Fix this ... just shows basemap with no predictions plotted.
# 
# Brian writes:
# 
# Here is some code which should remove the data points in the test_grid which
# are over water. It gets the coordinates for LA County boundary and then
# subsets test_grid to those points within the county boundary. The county does
# not include the water. So, this should work, but it does not because the
# resulting subset has zero rows for some reason. The sp::over() function is
# returning all NAs, but it should not.
