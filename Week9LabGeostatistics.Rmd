...
fall <- snapshot %>%
    ...
    select(ID, latitude, longitude, lat_m, long_m, ln_nox, D2A1, A1_50, A23_400, Pop_5000, D2C, Int_3000, D2Comm, cluster, group_loc, FieldID)

fall_geo <- as.geodata(fall, coords.col = 5:4, data.col = 6, covar.col = 7:13, covar.names = names(fall)[7:13])
...
par(mfrow = c(1,2))
points(fall_geo, ylab = "Lat_m", xlab = "Long_m", main = "Fall Snapshot Data:  ln(NOx)")

points(fall_geo, ylab = "Lat_m", xlab = "Long_m", main = "Fall Snapshot Data:  ln(NOx)", pt.divide = "quintile")
...
```{r convert LA grid to geodata, eval = FALSE}
# convert LA grid to geodata --------
# TODO:  Revise with the new grid; this grid doesn't have the right coordinates
