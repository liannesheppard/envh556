...
# Expand grid into a data frame and rescale predictions
test_grid_dens_expand<-
    with(test_grid_interp, expand.grid(x = x, y = y)) %>%
    mutate(z = as.vector(test_grid_interp$z),
           z = ifelse(is.na(z), 0, z),
           z = z * max(test_grid$pred_nox)/max(z))

g + stat_contour(aes(x = x, y = y, z = z, fill = ..level..), alpha = 0.08, 
                 data = test_grid_dens_expand, geom = "polygon", bins = 30) + 
