...
# sp: spatial data methods
# rgdal
# downloader
pacman::p_load(tidyverse, knitr, dplyr, geoR, maps, scatterplot3d,ggmap, 
...
A semivariance is defined as one half of the average of all the squared differences of all points that are a certain distance $h$ apart.  A variogram is a plot of these squared differences as a function of distance, presented as either all the points (a "cloud"), or as an average (typically called "binned").  Empirical variograms are useful for helping a data analyst see the spatial structure in the data.  In many situations with spatial structure, the average semivariance increases as distance increases until it eventually levels off.  In geostatistics we use a model to approximate the structure we see in an empirical variogram.  There are different variogram models depending upon the assumed covariance model.  Examples of common assumed models are exponential, spherical, Gaussian, and Matern.  This [link](http://www.kgs.ku.edu/Tis/surf3/s3krig2.html) gives a brief summary of semivariance and variograms (which it calls semivariograms; see the following link for discussion of terminology and the confusing overlapping terms in the literature).

There is confusion about terminology and often variogram and semivariogram are used interchangeably.  This paper, [Variogram or semivariogram? Understanding the variances in a variogram](https://link.springer.com/article/10.1007%2Fs11119-008-9056-2), gives a nice overview of the terminology and notation.  It also links well-understood estimates of variance with semivariance estimates, making the point that a variance can be expressed either in the traditional way as the squared deviation from the mean, or as 1/2 the average of squared differences between points, averaged over all possible pairs of points.  This helps you see that the formula for a variogram is a re-expression of a standard variance formula, though now looking at each term that goes into the sum as a function of distance.
...
# TODO:  this is a crude approach; look into ColorBrewer or other palette for a 
#   more sophisticated set of choices
col.range <- c("darkblue","blue","green","yellow","orange","red")

# add prediction plots, 3 types:
# Set xlim, ylim to ensure all plots are square
...
For later use, we need to convert the grid to geodata.  Also, the full grid isn't working, so we use a subset.  TODO:  Ask Brian?: (Optional!) Figure out if we can make this work by dividing the prediction step into two pieces.
...
This refinement is the same gridded predictions, but now drops all points over the water.

```{r plot gridded predictions not on the water, eval = TRUE}
# ------------ plot gridded predictions not on the water ---------------
#TODO: Ask Brian?: No overlaid plot with +.  Not working interactively at all once I added %+%

# identifies locations over the water and excludes them in spdf_test_grid_in_la
...
g %+% geom_point(aes(x = longitude, y = latitude, col = pred_nox), 
               data = test_grid_in_la, alpha = 0.7) + 
...
```{r plot smooth grid predictions on the map with with polygons}
...
Now show an example plotting smooth gridded predicitons on the map with tiles.  First tile example using stat_density_2d.  TODO:  Ask Brian?: Does this work, or should I remove it??
```{r plot smooth grid predictions on the map with tiles 1, eval = FALSE}
# ------------ plot smooth grid predictions on the map with tiles 1 -------

# Old code: Just shows a grey box over the area. TODO: Ask Brian?: Fix this.
# g + stat_density_2d(aes(x = longitude, y = latitude, fill = pred_nox), 
#                     data = test_grid, geom = "tile", contour = FALSE, 
#                     alpha = 0.7) + 
#     scale_fill_gradient(name = "NOx (ppb)", low = "yellow", high = "red") +  
#     ggtitle("Map of Los Angeles \n with fall UK predictions overlaid as tiles") + 
#     my_theme

# new try:  Still shows a gray box over the area, plus a legend
g %+% test_grid + aes(x = longitude, y = latitude, fill = pred_nox) +
...
Second tiles example uses stat_summary_2d. 
```{r plot smooth grid predictions on the map with tiles 2, eval = TRUE}
...
```{r prediction map with shapefiles, eval = TRUE}
#----- prediction map with shapefiles --------

# need 3 specific libraries in this chunk: rgdal, sp, downloader
