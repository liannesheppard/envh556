...
TODO:  Create versions that allow full datasets to be output from the program, at least when it is run one time.  Probably as an option to set.
...
# of functions to allow the paralell processing as in the following steps:
...
# The result returned from mesim_pure.R is a list of lists.  At the first level
# we have n_iter lists where each of those contains a list for each of the 7
# exposures (in predictors), and the 4 returned parameters for each.  We would
# like to convert this into a structure that is easier to analyze.  The next two
# conversion steps accomplish this:

# 1. Transform the 3D list of vectors into a 2D list of tibbles
res.lst.tbl <- lapply(1:n_iter, function(x) { 
    res.tbl <- as_tibble(matrix(unlist(result_pure_sp[[x]]), 
...
res <- res.tbl %>% 
    ...
    dplyr::summarize(
        ...
        E_b1 = mean(b1),
        sd_b1 = sd(b1),
        E_se_b1 = sqrt(mean(seb1^2)),
        E_R2 = mean(R2),
        E_exp_var = mean(exp_var)
        ) %>%
   arrange(E_exp_var) 
    
#    tidyr::gather(stat, val, -predictor) %>% 
#    tidyr::spread(predictor, val) %>% 
#    select("stat", predictors)
...
```{r ME simulation_like, eval = FALSE}
...
# Multi-core processing - Note: mclapply() does not work in Windows
# if (.Platform$OS.type != "windows") {
#     system.time(result_pure_mp <- mclapply(1:n_iter, function(x) me_like(), 
#                                            mc.cores = 8, 
#                                            mc.set.seed = TRUE,
#                                            mc.preschedule = TRUE))
#}
...
# This example finds the mean and sd of "b1" for all 5 predictors
# Note:  Students need to expand this summary to also get the E(se(beta1)), mean
# R2, and average variance of each exposure (ideally reported on the SD scale),
# plus analyze the other features and develop plots
res <- res.tbl %>% 
    group_by(predictor) %>% 
...
2.  In lab we will read through the `mesim_pure` function together to make sure you understand each step.  We can practice on one interation so you can investigate the full results.
TODO
