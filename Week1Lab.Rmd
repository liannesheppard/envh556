...
<!--
TODO:  Modify the loading of packages so ones loaded aren't reloaded.  Is this 
fixed now given we are using cache?
-->
---
...
##TODO: ADD other setup defaults including loading libraries PLUS other ENVH556 
##      standards??
...
## Run as a code chunk when testing.  When knitr is run, this is effectively run 
...
<!--Note: need a new line for R Markdown to recognize the following is a 
bulleted list -->
...
```{r datasets.path}
#------datasets.path--------
dir.create(file.path(ProjectPath, "Datasets"), showWarnings=FALSE, 
           recursive = TRUE)
...
i) How many observations are in your dataset?  Does this number correspond to
...
<!--Note:  Need a new line for R Markdown to recognize I'm starting a list-->
...
(@) `ggplot2` (part of `tidyverse`)  has great tools for plotting.  First we show
  a basic histogram.  Then we switch to the density scale and overlay a normal
  density with the same mean and variance as the data.  Finally we  overlay a
  density plot.
...
(@) `mutate( DEMS, ln_ecdata = log(ecdata) )` creates the natural log-transformed
...
## More plots:  Try out some scatterplots
...
### Now plot the relationship between NO (nodata)  and REC (ecdata). 
...
# Plot 3 adds a best fit line added and no 95% CI on the line (se=FALSE option).
# We also add a title and axis labels
ggplot(data = DEMS, aes(nodata, ecdata, na.rm = TRUE)) +
    geom_point() + stat_smooth(method = lm, se = FALSE) +
    labs(title = "Scatterplot of the DEMS NO (ppm) vs. REC (ug/m3) data",  x =
    "NO (ppm)",
    y = "REC (ug/m3)")
...
(1) Make table(s) summarizing four exposure variables in this dataset:  Nitrogen
    ...
    (b) Getting GM and/or GSD in a table in R:  I don't yet know how to get the
         GM and GSD using `tidyverse`, though it should be possible using a
         combination of `mutate` and `dplyr`.  You can use the `EnvStats` 
         package with `tapply` and `cbind`.  To get the AM, ASD, GM, GSD, N in 
         order:
```{r table.with.GM+GSD}
...
### A1.2 Base R option for a histogram of ecdata, also with a density curve added to the histogram. {-}
