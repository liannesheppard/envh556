...
TODO:  My interactive running of the following isn't working.  Error says "Error
in readRDS(dest) : error reading from connection"  Probably because I'm not
connected to the internet and at least one package isn't installed locally.
...
TODO:  Finish lecture slides (separate document) 
TODO:  Finish the bias-variance plot 
TODO:  Learn about and try out kableExtra for tables (see Nancy's week 1 lab for
an example) 
TODO:  install and test caret and cvTools for other cv approaches OR DROP 
...
* Extract predictions, and plot them:  (Note: the `fig.width` and `fig.height`
...
allows use of the F-test or a Chi-square test as alternatives.
...
minimizes the AIC and the option `k` controls the degree of penalty.  Since we
don't want to stop early, we use `k = 0` which seems to not impose a stopping
criterion.  More research is needed to understand exactly how the `k` option
operates w.r.t. stopping the alorithm.
...
# create the larges possible model, a full model that includes all of the
...
Note:  We are summarzing the cross-validation results overall.  This is one
valid way to do the summary.  Other approaches summarize the results in each
cluster (or cross-validation group) separately and average these.  (See e.g.
ISLR textbook.)
TODO:  Link to ISLR textbook
...
for (i in 1:10){
    ...
    # save the predictions and observations in CV.pred for the current cluster
    # (test set)
    CV.results <- fall %>%
...
paste("MSE-based R2:  ", round( max(1 - MSE.pred/MSE.obs, 0), 2))
...
From the *R for Data Science* (R4DS) chapter on functions, Chapter 15, the
...
arguments, there are key formatting details to follow.  In particular, pay
attention to the arguments inside the parentheses in the function command, and
...
TODO:  See other comments in the test chunk below
TODO:  Get suggestions from Brian on how to write this better.

```{r CV function}
# CV function -------
do_CV <- function(data, group, formula) {
    # TODO: probably need to also add the group variable to the dataset in case
    # it is not already part of the dataset OR do like the manual CV and only
    # output certain variables.  (However, then we risk not being able to do
    # more with the results if we wish.)
    ...
    # create a variable to store CV predictions 
    #CV.pred <- NULL
    # This version appends the new variable to the input data frame
    CV.pred <- data.frame(data, cvpreds = numeric(dim(data)[1]))
    ...
    for (i in 1:k){
        ...
        training <- data[!is.group, ]
        
        # fit the linear model to the training set
        CV.lm <- lm(formula = formula, data = training)
  
        # save the predictions and observations in CV.pred for the current group
        # (test set)
        tempCV <- data %>% modelr::add_predictions(CV.lm, "cvpreds") 
        CV.pred$cvpreds[is.group] <- tempCV$cvpreds[is.group]
    }
    # return the dataset
    CV.pred
}
...
# Note: the do_CV function only works if the group variable is a vector and not
# part of a dataset. 
# TODO: get rid of the above feature if straightforward should either append the
# grouping variable to the output dataset for cases when it isn't part of the
# input dataset, or require that it is a variable in the dataset
temp <- do_CV(data = fall, group = fall$cluster, formula = common_model_formula)
...
TODO:  UPDATE --- this code seems to work, but the do_CV isn't giving the right results
...
res <-
    data.frame(
        ...
        in_MSE           = numeric(n_covars),
        in_R2            = numeric(n_covars),
        out_MSE          = numeric(n_covars),
...
```{r plot bias-var plot, eval=FALSE}

# David's code for ggplot; needs to be revised
# plots with both model based (in sample) R2 and CV generated MSER2

plot.R2 <- ggplot(results_store) +
    geom_point(aes(x = i, y = in_R2, color = "blue")) + 
    geom_smooth(aes(x = i, y = in_R2, color = "blue"), se = F) +
    geom_point(aes(x = i, y = out_R2, color = "red")) + 
    geom_smooth(aes(x = i, y = out_R2, color = "red"), se = F) +
...
# scale_color_discrete(name="Source",guide = guide_legend(reverse=TRUE))
...
this lab you should restrict your analysis to one season (e.g. winter).
3.	Fit the model for one of the seasons given in Table 4 of Mercer et al. Make
note of these in-sample estimates of R2 and RMSE.
4.	Try to manually cross-validate this model using the code given above.
Compare the CV R2 and RMSE to your in-sample estimates.  (Note:  Use the cluster
...
with a lax entry criterion (e.g. ADD).   (You may restrict your attention to the
list in the forward selection model example given above.)  Keep track of the
order the variables were added.
...
along with the R2 and/or RMSE estimates from CV and the training data.   If you
choose to show your results in a plot, put the number of variables in the model
on the x-axis vs. the R2 or RMSE estimates on the y-axis.  Distinguish the two
...
1.	Write a brief summary of the purpose of the lab and your approach.  Then present your results:
    ...
    on how your in-sample and cross-validated MSE-based R2 estimates compare.
...
# Appendix 1:  Other resources to explore
...
NOTE:  This has not been tested yet.  We need to determine whether this package
gives the same results as our manual procedures, or not.  Also, the worked
example below is randomly choosing CV groups, which is a very different approach
from using the cluster variable.

Of the three commonly used cross-validation methods (random subsampling, k-fold
cross-validation and leave-one-out cross-validation), here we will focus on
k-fold cross-validation. For more details on cross-validation, please refer to
the **Lecture Notes** and
<https://www.r-bloggers.com/cross-validation-for-predictive-analytics-using-r/>.

The following uses tools and examples from *cvTools* package to write a program
to cross-validate results from our linear model.
...
Set-up folds for cross-validation, using random allocation into groups.  (Recall
the snapshot data has clusters already assigned that keep road gradient sites
together.)
...
The `caret` package has code for k-fold cross-validation, as does the `boot`
package.  and the `cvTools` package.  Not sure which to prioritize.
