...
do_CV <- function(obs, formula, group, data) {
    ...
    for (i in 1:k){
        ...
        # subset the data  by omitting cluster i to create the training set
...
temp <- do_CV(obs = CV.pred$ln_nox, 
              formula = common_model_formula, 
              group = fall$cluster, 
              data = fall)
...
res <- data.frame(n_pred = numeric(n_covars), covar = character(n_covars), in_MSE = numeric(n_covars), in_R2 = numeric(n_covars), out_MSE = numeric(n_covars), out_R2 = numeric(n_covars), stringsAsFactors = FALSE)
...
for (i in 1:n_covars) {
    ...
    fmla <- as.formula(paste("ln_nox ~ 1 + ", paste(covars_forward[1:i], collapse = "+"))) 
...
plot.R2 <- ggplot(results_store) +
    geom_point(aes(x=i, y=in_R2, color="blue")) +         geom_smooth(aes(x=i, y=in_R2, color="blue"), se=F) +
    geom_point(aes(x=i, y=out_R2, color="red")) +         geom_smooth(aes(x=i, y=out_R2, color="red"), se=F) +
    xlab("Model Complexity (# of terms)") + 
    ylab(bquote(bold(R^2))) +
    scale_x_continuous(breaks=c(seq(0,63,5))) +
    theme_bw() 
#+ theme(axis.text=element_text(face="bold"),axis.title=element_text(face="bold")) + scale_color_discrete(name="Source",guide = guide_legend(reverse=TRUE))
