...
ggplot(DEMS, aes(x = facilityno, y = ln_ecdata)) +
    ...
    labs(title = "Boxplots with Overlaid Dotplots\nfor Ln(REC) by Facility Number",
         ...
         x = "Facility ID")
```
## Slide 10b:  Stratified by facility ID
...
TODO:  Think about focus of this result in comparison to the random effect result.  Perhaps use the "lincom" approch to estimate means and then estimate their variances?? Or just focus on the residual MSE estimate?
...
Think about what to emphasize here
...
VCA allows the MOM estimates and easy comparison with REML estimates.  Also want to compare the random effect analysis vs. fixed effect w.r.t. estimates of variation.  Consider doing a lincom of coefficients and then estimating the same variability of the means we did by hand.
...
Want to make the point here that the lmer REML estimates are the same as from VCA.  Discuss why we use ML at all -- basicially to easily compare models.
...
## Slides 26 + 27:  DEMS with VCA

Decide whether to do all of these for the DEMS data or be more selective.
...
## Slide 28:  DEMS comparison of estimates

Did not redo the R coding for this.  Would need to use the code for the snapshot data, now applied to DEMS, in this section

## Slide 28?:  Mixed model with the Snapshot data

Address how the two variance components changed.  Discuss whether we expect the impact on what scale.

```{r}
mixedfit1_REML <- lmer(ln_nox ~ Pop_5000 + (1|group_loc), data = fvc, REML = TRUE)
summary(mixedfit1_REML)
```
## Slide 29?:  Mixed model with the Snapshot data: two covariates

TODO:  discuss model selection using ML

Need to pay attention to whether you use ML or REML for comparisons.  Basically to compare models you need to use ML because of how the variance components are estimated to be unbiased in REML.  For final estimates we should report REML.

```{r}
mixedfit2_REML <- lmer(ln_nox ~ Pop_5000 + D2Comm + (1|group_loc), data = fvc, REML = TRUE)
summary(mixedfit2_REML)
```


## Slide 29?:  DEMS V Table 3 Facility A example U1 groups

THis result has the same number of observations (108) and mdj groups (13) compared to the Stata example from lecture.  My mean is about the same (5.85).  This seems to be the same dataset
...
## Slide 37?:  Regression for association with random effects

TODO:  Use the snapshot data to compare estimates and SEs for a fixed effect with and without random effects
...
```
## Slide 38?:  Regression for prediction with random effects
...
TODO:
* make a new dataset that is the first observation per cluster
* predict
